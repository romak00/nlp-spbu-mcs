...
class LoraLayer(Module):
    ...
    def __init__(self, in_features: int, out_features: int, r: int):
        ...
        """
        super().__init__()
        self.A = Linear(in_features=in_features, out_features=r, bias=False)
        self.B = Linear(in_features=r, out_features=out_features, bias=False)
        self.B.weight.data = torch.zeros(self.B.weight.data.shape)
